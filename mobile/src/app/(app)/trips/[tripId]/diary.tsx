...
export default function DiaryScreen() {
  ...
  return <SafeAreaView style={topSafeAreaStyle} edges={['top', 'left', 'right']}>
    ...
    <FlatList style={bodyStyle} data={entries} keyExtractor={item => item.id} refreshing={refreshing} onRefresh={() => void loadEntries()} contentContainerStyle={styles.content}
      ...
      renderItem={({ item }) => <Pressable onPress={() => setEditing(item)} style={styles.card}><View style={styles.cardTop}><Text style={styles.cardTitle}>{item.title}</Text><Pressable onPress={() => confirmDelete(item)}><Text style={styles.delete}>삭제</Text></Pressable></View><Text numberOfLines={3} style={styles.preview}>{item.content}</Text><View style={styles.tags}>{item.itinerary ? <Text style={styles.tag}>일정 · {item.itinerary.name}</Text> : null}{item.place ? <Text style={styles.tag}>장소 · {item.place.name}</Text> : null}</View><Text style={styles.updated}>{new Date(item.updatedAt).toLocaleString()}</Text><ImageGallery target={{ tripId: trip.id, dayId: item.tripDayId, entryId: item.id }} refreshKey={`${imageRefresh}:${imageVersions[item.id] ?? 0}`} /><Pressable accessibilityRole="button" onPress={event => { event.stopPropagation(); setPhotoEntry(item); }}><Text style={styles.photoLink}>＋ 사진 추가</Text></Pressable></Pressable>} />
